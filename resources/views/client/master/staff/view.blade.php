...
@push('BreadCrumbMenu')
   <li>Master</li>
   <li class="active"><a href="{{ action('ClientController\StaffController@index') }}">Staff</a></li>
@endpush
...
@section('content')
    ...
    <div class="row">
        <div class="col-xs-12">
             ...
             @component('client.layout.component.panel-head',['MenuTitle'=>'Staff','Title'=>'Staff List','color'=>env('TABPANELCOLOR')])
                @slot('Button')
                    <a href="{{ action('ClientController\StaffController@create') }}" class="btn btn-info btn-sm"><i class="fa fa-plus"></i> Staff</a>
                @endslot
                        ...
                        <tbody>
                            @foreach(auth()->user()->staffs as $Staff)
                                <tr>
                                    <td>{{ $Staff->name }}</td>
                                    <td>{{ $Staff->mobile1 }}</td>
                                    <td>{{ substr($Staff->address,0,30) }}</td>
