...
class IncomeController extends Controller
{
    public function IncomeBalanceCustomerList(){
        return view('manager.income.incomeBalanceCustomerList');
    }
    ...
    public function AddCustomerIncome($CustomerId)
    {
        ...
        if (!empty($final_data)) {
            return view('manager.income.CustomerIncomeBalanceList', $Data, compact('final_data', 'customer', 'total'));
        } else {
    ...
    public function SaveCustomerIncome($customerId){
        ...
        foreach (request()->income as $vehicleEntryId => $incomeDetail){
            if(!empty($incomeDetail['recevingAmount']) || !empty($incomeDetail['discountAmount'])){
                ...
                $income->clientid= auth()->user()->owner->id;
                $income->managerid= auth()->user()->id;
        ...
        return redirect(route('manager.IncomeBalanceCustomerList'))->with('success',['Income','Added Sucessfully!']);
    }

    public function view(){
        return view('manager.income.view');
    }

    public function edit($id){
        try {
            $Data['Income'] = Income::findOrfail($id);
            return view('manager.income.edit',$Data);
        }catch (Exception $e){
