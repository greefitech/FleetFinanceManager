...
class MemoController extends Controller
{
    ...
    public function memo(){
        $Data['RTOMasters'] = $this->RTOMaster::where([['clientid',auth()->user()->owner->id]])->get();
        return view('manager.trip.memo',$Data);
    }
        ...
        try {
            ...
            if(!empty(request('DieselData'))){
                foreach(request('DieselData')['amount'] as $DieselDataKey=>$Dis){
                    ...
                    $Expense->clientid=auth()->user()->owner->id;
                    $Expense->managerid = auth()->user()->id;
                    $Expense->save();
                }
            }
