...
class FinancialIndicatorController extends Controller
{
    public function add($VehicleId){
        ...
        return view('manager.master.vehicle.financial-indicator.add',$Data);
    }
    ...
    public function save($VehicleId){
        ...
        try {
            ...
            $FinantcialIndicator->managerid = auth()->user()->id;
            $FinantcialIndicator-> save();
            return redirect(route('manager.ViewVehicles'))->with('success',['Financial Indicators','Created Successfully!']);
    ...
    public function view($FinancialId){
        ...
        return view('manager.master.vehicle.financial-indicator.view',$Data);
    }

    public function edit($FinancialId){
        $Data['FinancialIndicator'] = FinancialIncdicator::findorfail($FinancialId);
        $Data['Vehicle'] = Vehicle::findorfail($Data['FinancialIndicator']->vehicleId);
        return view('manager.master.vehicle.financial-indicator.edit',$Data);
    }
    ...
    public function update($VehicleId,$FinancialID){
        ...
        try {
            ...
            $FinantcialIndicator->managerid = auth()->user()->id;
            $FinantcialIndicator-> save();
            return redirect(route('manager.ViewVehicles'))->with('success',['Financial Indicators','Updated Successfully!']);
        } catch (\Illuminate\Database\QueryException $e) {
            return back()->with('danger','Something went wrong!! Finantial Indicator Already Added!');
        }
    }
