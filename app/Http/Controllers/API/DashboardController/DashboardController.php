...
class DashboardController extends Controller
{
	...
	*/
    ...
    public function dashboardLastThreeMonthChart(){
        ...
        $success['chart'][2]['month'] = $previous2month;
        $success['chart'][2]['profit'] = auth()->user()->CalculateProfitAmountTotal('',$prev2month,$prev2year);
        $success['chart'][2]['expense'] = auth()->user()->CalculateNonTripExpenseAmountTotal('',$prev2month,$prev2year);


        $previousmonth = date("M", mktime(0, 0, 0, $prevmonth,10)).' - '.$prevyear;
        $success['chart'][0]['month'] = $previousmonth;
        $success['chart'][0]['profit'] = auth()->user()->CalculateProfitAmountTotal('',$prevmonth,$prevyear);
        $success['chart'][0]['expense'] = auth()->user()->CalculateNonTripExpenseAmountTotal('',$prevmonth,$prevyear);


        $currentmonth = date("M", mktime(0, 0, 0, $month,10)).' - '.$year;
        $success['chart'][1]['month'] = $currentmonth;
        $success['chart'][1]['profit'] = auth()->user()->CalculateProfitAmountTotal('',$month,$year);
        $success['chart'][1]['expense'] = auth()->user()->CalculateNonTripExpenseAmountTotal('',$month,$year);
